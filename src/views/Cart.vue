        ...
        <div class="column is-9">
          <div class="columns" v-for="item in productsCart" :key="item.id">
            <div class="column is-3">
              <b-image :src="`https://raw.githubusercontent.com/almogrote/images/main${item.image}`" ratio="16by9"></b-image>
            </div>
            ...
            <div class="column is-3">
              <div class="has-background-light p-4 h-100">
                ...
                <p class="is-size-5 has-text-danger-dark has-text-weight-light">{{ item.amount }} € <b-numberinput type="is-dark" :value="item.quantity" min="1" max="10" :editable="false" controls-position="compact" /></p>
              </div>
            </div>
            <div class="column is-2">
              <div class="has-background-light p-4 h-100">
                <h3 class="title is-4">Total</h3>
                <p class="is-size-5 has-text-danger-dark has-text-weight-light">{{ totalProductPrice(item.quantity, item.amount).toFixed(2) }}€</p>
              </div>
            </div>
          </div>
          <hr />
        ...
        <div class="column is-3">
          <div class="content has-background-light p-4">
            ...
            <p class="title has-text-centered">
              Total: <span class="has-text-weight-light has-text-danger-dark is-size-3">{{ cartTotal.toFixed(2) }} €</span>
            </p>
            <StripeCheckout />
          </div>
        </div>
